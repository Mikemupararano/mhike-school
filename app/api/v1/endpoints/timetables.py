...
@router.post("/periods", response_model=TimetablePeriodOut)
...
@router.get("/teacher/me", response_model=list[TimetableEntryOut])
...
@router.get("/student/me", response_model=list[TimetableEntryOut])
async def get_my_student_timetable(
    class_group_id: int | None = Query(default=None),
    day_of_week: str | None = Query(default=None),
    limit: int = Query(default=100, le=200),
    offset: int = Query(default=0, ge=0),
    ...
    PermissionService.ensure_active_user(current_user)
    PermissionService.ensure_student(current_user)
...
@router.get("/assignments", response_model=list[TimetableAssignmentOut])
