...
def _required_date(
    ...
    Staged import rows are stored as JSON, so validated dates normally arrive
    as ISO-formatted strings such as ``2026-09-01``. Direct processor calls
    may still provide ``datetime.date`` instances.
    """
...
def _optional_boolean(
    ...
    *,
    ...
    The processor therefore requires an actual bool when the field exists.
    """
...
async def process_timetable_row(
    ...
    Existing timetables are matched using:
    ...
    await repository.save_timetable(
        existing_timetable,
    )
