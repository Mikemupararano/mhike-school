...
def _should_update_existing_records(
    ...
    """
    Return whether existing teacher records may be modified.

    ``None`` preserves the historical behaviour for direct processor calls
    made outside the generic import-batch framework.

    When a batch supplies an options mapping, updating existing records is
    opt-in and defaults to False if the option is absent.
    """
...
def _is_existing_teacher(
    ...
    when updates are permitted without converting unrelated student or parent
    accounts.
    """
...
async def process_teacher_row(
    ...
    """
    Create, update or skip one teacher from validated import data.
    ...
    Behaviour for existing teachers is controlled by the batch-level
    ``update_existing_records`` option:

        True:
            Existing teacher records may be updated.

        False:
            Existing teacher records are left unchanged and the row returns
    ...
    primary-role field. Unrelated student and parent accounts are never
    converted automatically.

    Legacy teacher accounts missing their authoritative ``user_roles``
    ...
    # ------------------------------------------------------------------
    # Existing Teacher: respect the administrator's update preference.
    # ------------------------------------------------------------------
    ...
    if not _should_update_existing_records(
        ...
        return RowProcessingResult(
            ...
            message=(
                f"Skipped existing teacher "
                f"'{existing_user.full_name or full_name}' because "
                "updating existing records is disabled."
            ),
        )

    # ------------------------------------------------------------------
    # Existing Teacher: update when explicitly permitted.
    # ------------------------------------------------------------------
