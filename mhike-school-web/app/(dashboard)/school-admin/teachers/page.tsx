...
function getUserDisplayName(user: User): string {
    return user.full_name?.trim() || "Unnamed teacher";
}
...
function TeachersContent() {
    ...
    const summary = useMemo(
        () => ({
            ...
            active: teachers.filter(isUserActive).length,
            inactive: teachers.filter(
                (teacher) => !isUserActive(teacher),
            ).length,
        }),
        [teachers],
    );
        ...
        <main className="min-h-full bg-slate-50 px-4 py-6 sm:px-6 lg:px-8">
            <div className="mx-auto w-full max-w-7xl">
                <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
                            Review teacher accounts, search the
                            staff directory and manage account
                            status.
                        </p>
                    </div>
                ...
                >
                    {isLoading
                        ? "Loading teachers."
                        : `${filteredTeachers.length} teachers displayed.`}
                </div>
                ...
                {isLoading ? (
                    ...
                    <>
                        <section
                            aria-labelledby="teacher-summary-heading"
                            className="mt-8"
                        >
                            <h2
                                id="teacher-summary-heading"
                                className="sr-only"
                            >
                                Teacher summary
                            </h2>
        ...
        <div className="px-6 py-14 text-center">
            ...
            <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
                {hasActiveFilters
                    ...
                    : "Create a teacher account to begin assigning staff to classes and courses."}
            </p>
